...
OUTPUT_DIRECTORY = './calib_images/output'  # Directory to save calibration results
...
def calibrate_camera():
    ...
    save_camera_poses_csv(rvecs, tvecs)
...
def save_camera_poses_csv(rvecs, tvecs):
    camera_poses = pd.DataFrame(columns=['image_id','rx','ry','rz','tx','ty','tz'])

    for image_id in range(len(rvecs)):
        row = [image_id] 
        row.extend(rvecs[image_id].flatten())
        row.extend(tvecs[image_id].flatten())
        print(row)
        camera_poses.loc[image_id] = row
    
    camera_poses.to_csv(os.path.join(OUTPUT_DIRECTORY, 'camera_poses.csv'))
